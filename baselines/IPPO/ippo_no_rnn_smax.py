...
#MODIFIED: removed the real heuristic enemy smax env in favor of idle version for testing
...
# Modified ActorCritic to remove RNN
class ActorCriticNoRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        # unavail_actions = jnp.zeros_like(avail_actions)
...
def make_train(config):
    ...
    def train(rng):
        # INIT NETWORK
        network = ActorCriticNoRNN(env.action_space(env.agents[0]).n, config=config)
        ...
        # Dummy hidden state
        init_hstate = jnp.zeros((1, config["NUM_ACTORS"], 1))  # Dummy hidden state
...
def main(config):
    ...
    with open("actor_network_params_facmac_mod.pkl", "wb") as f:
