...
if __name__ == "__main__":
    ...
    sa_encoder_params = sa_encoder.init(sa_key, np.ones((1,) + env_state.info['world_state'].shape[1:]), np.ones([1, action_size]))
    g_encoder = G_encoder()
    ...
    dummy_world_state = jnp.zeros_like(env_state.info['world_state'][0])
    ...
    def deterministic_actor_step(training_state, env, env_state, extra_fields):
        ...
        return nstate, Transition(
            observation=obs,
            world_state=jnp.repeat(env_state.info['world_state'], args.num_agents),
            action=actions,
...
#        jax.debug.print("actions from first env: {}", transition_actions[0])
        ...
        state_extras = {x: jnp.repeat(nstate.info[x], args.num_agents) for x in extra_fields}
#        print(env_state.info['world_state'].shape)

        return nstate, Transition(
            observation=obs,
            world_state=jnp.repeat(env_state.info['world_state'], args.num_agents, axis=0),
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            world_state = transitions.world_state
...
#                jax.debug.print("actions on sampled obs: {}", means[:6,:])
            ...
            sa_repr = sa_encoder.apply(sa_encoder_params, world_state, action)
