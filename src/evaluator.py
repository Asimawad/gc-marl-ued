...
def generate_unroll(actor_step, training_state, env, env_state, unroll_length, extra_fields=(), key=None):
    """Collect trajectories of given unroll_length."""

    @jax.jit
    def f(carry, unused_t):
        state = carry
        nstate, transition = actor_step(training_state, env, state, extra_fields=extra_fields, key=key)
...
class CrlEvaluator():
    def __init__(self, actor_step, eval_env, num_eval_envs, episode_length, key):
        ...
        def generate_eval_unroll(training_state, key):
            reset_key, step_key = jax.random.split(key, 2)
            ...
            return generate_unroll(
                ...
                eval_env,
                eval_first_state,
                unroll_length=episode_length, key=step_key)[0]
