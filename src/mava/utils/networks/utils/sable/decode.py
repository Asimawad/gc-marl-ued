...
def get_shifted_discrete_actions(action: chex.Array, legal_actions: chex.Array, n_agents: int) -> chex.Array:
...
def discrete_autoregressive_act(
    ...
    for i in range(N):
        ...
        shifted_actions = shifted_actions.at[:, i + 1, 1:].set(jax.nn.one_hot(action[:, 0], A), mode="drop")
...
def get_shifted_continuous_actions(action: chex.Array, action_dim: int, n_agents: int) -> chex.Array:
