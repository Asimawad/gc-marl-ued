...
class EncodeBlock(nn.Module):
    ...
    def __call__(self, x: chex.Array, hstate: chex.Array, dones: chex.Array, step_count: chex.Array) -> chex.Array:
        """Applies Chunkwise MultiScaleRetention."""
        ret, updated_hstate = self.retn(key=x, query=x, value=x, hstate=hstate, dones=dones, step_count=step_count)
        x = self.ln1(x + ret)
    ...
    def recurrent(self, x: chex.Array, hstate: chex.Array, step_count: chex.Array) -> chex.Array:
        ...
        ret, updated_hstate = self.retn.recurrent(key_n=x, query_n=x, value_n=x, hstate=hstate, step_count=step_count)
        x = self.ln1(x + ret)
        output = self.ln2(x + self.ffn(x))
        return output, updated_hstate
...
class Encoder(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.obs_encoder = nn.Sequential(
            [
                nn.RMSNorm(),
                nn.Dense(self.net_config.embed_dim, kernel_init=orthogonal(jnp.sqrt(2)), use_bias=False),
                nn.gelu,
            ],
        )
...
class DecodeBlock(nn.Module):
    ...
    def __call__(
        ...
        ret, hs1_new = self.retn1(key=x, query=x, value=x, hstate=hs1, dones=dones, step_count=step_count)
        ret = self.ln1(x + ret)
    ...
    def recurrent(
        ...
        # Apply the self-retention over actions
        ret, hs1_new = self.retn1.recurrent(key_n=x, query_n=x, value_n=x, hstate=hs1, step_count=step_count)
        ret = self.ln1(x + ret)

        # Apply the cross-retention over obs x action
        ret2, hs2_new = self.retn2.recurrent(key_n=ret, query_n=obs_rep, value_n=ret, hstate=hs2, step_count=step_count)
        y = self.ln2(obs_rep + ret2)
        output = self.ln3(y + self.ffn(y))
...
class Decoder(nn.Module):
    ...
    def __call__(
        ...
        for i, block in enumerate(self.blocks):
            ...
            updated_hstates = tree.map(lambda x, y, j=i: x.at[:, :, j].set(y), updated_hstates, hs_new)
...
class SableNetwork(nn.Module):
    ...
    def setup(self) -> None:
        ...
        assert (
            self.memory_config.decay_scaling_factor >= 0 and self.memory_config.decay_scaling_factor <= 1
        ), "Decay scaling factor should be between 0 and 1"

        # Decay kappa for each head
        self.decay_kappas = 1 - jnp.exp(jnp.linspace(jnp.log(1 / 32), jnp.log(1 / 512), self.net_config.n_head))
        ...
        if self.action_space_type == _CONTINUOUS:
            ...
            self.autoregressive_act = partial(continuous_autoregressive_act, action_dim=self.action_dim)
        else:
            self.train_decoder_fn = partial(
                discrete_train_decoder_fn,
                n_agents=self.n_agents,
