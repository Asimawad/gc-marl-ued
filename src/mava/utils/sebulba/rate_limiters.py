...
class RateLimiter:
    ...
    def __init__(self, samples_per_insert: float, min_size_to_sample: float, min_diff: float, max_diff: float):
...
class SampleToInsertRatio(RateLimiter):
    ...
    def __init__(
        ...
        if min_size_to_sample < 1:
            raise ValueError(f"min_size_to_sample ({min_size_to_sample}) must be a positive integer")
