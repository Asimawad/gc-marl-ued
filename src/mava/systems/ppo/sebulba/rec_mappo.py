        ...
        with RecordTimeTo(actor_timings["rollout_time"]):
            for _ in range(config.system.rollout_length):
                ...
                with RecordTimeTo(actor_timings["compute_action_time"]):
                    ...
                    action, log_prob, value, hstates = act_fn(params, last_obs, last_dones, last_hstates, act_key)
...
def get_learner_step_fn(
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple[Tuple, Metrics]:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                actor_loss_info, actor_grads = actor_grad_fn(params.actor_params, traj_batch, advantages, entropy_key)
            ...
            permutation = jax.random.permutation(shuffle_key, num_learner_envs * num_recurrent_chunks)
    ...
    def learner_fn(learner_state: RNNLearnerState, traj_batch: RNNPPOTransition) -> Tuple[RNNLearnerState, Metrics]:
        """Learner function.
        ...
        with RecordTimeTo(learn_times["learner_time_per_eval"]):
            for _ in range(config.system.num_updates_per_eval):
                ...
                learner_state = learner_state._replace(
                    timestep=timestep,
                    dones=timestep.last().repeat(config.system.num_agents).reshape(config.arch.num_envs, -1),
...
def learner_setup(
    ...
    learn_state_spec = RNNLearnerState(model_spec, model_spec, data_spec, None, data_spec, data_spec, data_spec)
