...
from mava.wrappers.episode_metrics import SFLRecordEpisodeMetrics, get_final_step_metrics
...
def get_env_metrics(state: State) -> Metrics:
    mean_manhatten_distance = jnp.mean(jnp.sum(jnp.abs(state.agents.start - state.agents.target), axis=-1))
    return {
...
def make_jumanji_env(config: DictConfig, add_global_state: bool = False) -> Tuple[MarlEnv, MarlEnv]:
    ...
    viewer = ConnectorViewer("Connector", config.env.scenario.task_config["num_agents"], render_mode="rgb_array")
...
def make_env(config: DictConfig) -> MarlEnv:
    ...
    train_env = SFLRecordEpisodeMetrics(train_env)
    eval_env = SFLRecordEpisodeMetrics(eval_env)
...
def get_learner_fn(
    ...
    def _update_step(
        ...
        def _env_step(
            ...
            critic_hidden_state, value = critic_apply_fn(params.critic_params, last_hstates.critic_hidden_state, ac_in)
            ...
            learner_state = RNNLearnerState(params, opt_states, key, env_state, timestep, done, hstates)
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                value_loss_info, critic_grads = critic_grad_fn(params.critic_params, traj_batch, targets)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    sampled_evaluator = get_sampled_eval_fn(eval_env, sampled_eval_keys, eval_act_fn, config, absolute_metric=False)
    ...
    for eval_step in range(config.arch.num_evaluation):
        ...
        success_scores, learnability_scores, learnable_instances, all_success_scores = get_learnability_set(
            learnable_key,
        ...
        # Evaluate.
        eval_metrics, lowest_wr_keys = sampled_evaluator(trained_params, eval_keys, {"hidden_state": eval_hs})
        logger.log(eval_metrics, eval_step * config.system.num_updates_per_eval, eval_step, LogEvent.EVAL)
...
def get_learnability_set(
    ...
    def _batch_step(_, rng):
        ...
        runner_state, traj_batch = jax.lax.scan(_env_step, runner_state, None, config.ued.rollout_steps)
    ...
    _, (success, learnability, env_state) = jax.lax.scan(_batch_step, None, rngs, config.ued.num_batches)
