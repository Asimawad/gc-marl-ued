...
from mava.networks.icrl import DiscreteICRLActor, DiscreteSAEncoder
...
def has_any_nan(params):
    return jax.tree.reduce(
        # Check if current leaf has NaN, OR it with the accumulator
...
def get_learner_fn(
    ...
    # For discrete actions, maximum entropy is log(num_actions) (uniform distribution)
    # target_entropy = -config.system.target_entropy_scale * jnp.log(action_dim)
    target_entropy = config.system.target_entropy_scale * jnp.log(action_dim)
    ...
    def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, ICRLTransition]:
        ...
        def single_step(carry, _):
            ...
            policy = actor_apply(params.actor, obs, action_mask)  # means: [N_env, N_agent, A]
            action = policy.sample(seed=policy_key)
            actions_onehot = jax.nn.one_hot(action, action_dim)
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _update_minibatch(carry, batch_transitions):
            ...
            def _critic_loss_fn(critic_params, obs, actions_onehot):
                ...
                actions = jnp.argmax(actions_onehot, axis=-1)  # [batch]

                # Compute representations
                state_repr = sa_encoder_apply(critic_params["sa_encoder"], state)
                g_repr = goal_encoder_apply(critic_params["goal_encoder"], goal)
                # TODO: should this action come from the current policy or the buffer
                state_action_repr = state_repr[jnp.arange(batch_size, dtype=jnp.int32), actions, :]
                # print(f"{state_repr.shape=} | {state_action_repr.shape=} | {g_repr.shape=}")

                # InfoNCE: compute pairwise distances with epsilon for numerical stability
                # logits[i,j] = -distance(sa_repr[i], g_repr[j])
                logits = -jnp.sqrt(
                    jnp.sum((state_action_repr[:, None, :] - g_repr[None, :, :]) ** 2, axis=-1) + 1e-8
                )  # [batch, batch]
            ...
            def _actor_loss_fn(
                ...
                probs = jax.nn.softmax(policy.distribution.logits)
                log_probs = jax.nn.log_softmax(policy.distribution.logits)

                # Compute Q-value (negative distance) with epsilon for numerical stability
                state_action_repr = sa_encoder_apply(critic_params["sa_encoder"], state)
                # Add new axis so when we multiply we keep the action dim and use it with
                # the log probs in the loss

                g_repr = goal_encoder_apply(critic_params["goal_encoder"], goal)[:, jnp.newaxis]
                q_value = -jnp.sqrt(jnp.sum((state_action_repr - g_repr) ** 2, axis=-1) + 1e-8)

                # print(f"{state_action_repr.shape=} {g_repr.shape=}")
                # print(f"{probs.shape=} {alpha.shape=} {log_probs.shape=} {q_value.shape=}")

                actor_loss = (probs * ((alpha * log_probs) - q_value)).sum(axis=-1).mean()
                entropy = -(probs * log_probs).sum(axis=-1).mean()

                loss_info = {"actor_loss": actor_loss, "entropy": entropy}
                return actor_loss, loss_info

            def _alpha_loss_fn(log_alpha, probs, log_probs):
                """Temperature loss for discrete SAC.

                Objective: L_α = α * (H_target - H(π))
                Expanded: L_α = α * Σ π(a) (log π(a) + H_target)
                """
                alpha = jnp.exp(log_alpha)
                # Correct formulation: no negative sign
                alpha_loss = (probs * (alpha * (log_probs + target_entropy))).sum(axis=-1).mean()
            ...
            (actor_loss, actor_info), actor_grads = actor_grad_fn(
                params.actor, critic_params, obs, alpha, avail_actions, future_state, actor_key
            )
            ...
            if config.system.autotune:
                # NOTE: this is done in the actor loss fn, we should re-use it
                state = obs[:, :obs_dim]
                goal = future_state[:, goal_start_idx:goal_end_idx]
                observation = jnp.concatenate([state, goal], axis=1)
                policy = actor_apply(new_actor, observation, avail_actions)
                probs = jax.nn.softmax(policy.distribution.logits)
                log_probs = jax.nn.log_softmax(policy.distribution.logits)
                alpha_grad_fn = jax.value_and_grad(_alpha_loss_fn, has_aux=True)
                (alpha_loss, alpha_info), alpha_grads = alpha_grad_fn(params.log_alpha, probs, log_probs)
...
def learner_setup(env: MarlEnv, keys: chex.Array, config: DictConfig) -> Tuple:
    ...
    actor_params = actor_network.init(actor_key, init_obs, init_action)

    # Initialize log_alpha
    if config.system.autotune:
        # when autotuning alpha starts at 1/log apha starts at 0
    ...
    actor_opt = optax.chain(grad_clip, optax.adam(config.system.policy_lr, eps=config.system.adam_eps))
    critic_opt = optax.chain(grad_clip, optax.adam(config.system.q_lr, eps=config.system.adam_eps))
    alpha_opt = optax.chain(grad_clip, optax.adam(config.system.alpha_lr, eps=config.system.adam_eps))
