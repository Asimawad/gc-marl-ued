...
LearnFnType = Callable[[LearnerState, Transition, HiddenStates, HiddenStates], Tuple[LearnerState, Metrics]]
    ...
    while not stop_event.is_set():
        ...
        with RecordTimeTo(actor_timings["rollout_time"]):
            for _ in range(config.system.rollout_length):
                ...
                prev_dones = np.repeat(timestep.last(), config.system.num_agents).reshape(config.arch.num_envs, -1)
                ...
                hstates = tree.map(lambda hs, dones=dones: np.where(dones, np.zeros_like(hs), hs), hstates)

                # Append data to storage
                traj.append(Transition(prev_dones, action, value, timestep.reward, log_prob, obs_tpu))
...
def get_learner_step_fn(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _loss_fn(
                    ...
                    total_loss = actor_loss - config.system.ent_coef * entropy + config.system.vf_coef * value_loss
    ...
    def learner_fn(
        ...
        learner_state, loss_info = _update_step(learner_state, traj_batch, initial_hstate, updated_hstates)
        ...
        with RecordTimeTo(learn_times["learner_time_per_eval"]):
            for _ in range(config.system.num_updates_per_eval):
                ...
                with RecordTimeTo(learn_times["learning_time"]):
                    learner_state, train_metrics = learn_fn(learner_state, traj_batch, initial_hstate, updated_hstates)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    # Setup learner.
    learn, select_action_fn, learner_state, learner_sharding = learner_setup(key, config, learner_devices)
    ...
    steps_per_rollout = config.system.rollout_length * config.arch.num_envs * config.system.num_updates_per_eval
