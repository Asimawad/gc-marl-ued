...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple[Transition, Metrics]]:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _guider_loss_fn(
                    ...
                    if isinstance(actor_policy, IdentityTransformation):
                        kl_loss = tfd.kl_divergence(guider_policy, lax.stop_gradient(actor_policy.distribution))
                    else:
                ...
                def _actor_loss_fn(
                    ...
                    # Use .distribution since tfd.kl_divergence can't handle IdentityTransformation
                    if isinstance(actor_policy, IdentityTransformation):
                        kl_loss = tfd.kl_divergence(lax.stop_gradient(guider_policy), actor_policy.distribution).mean()
                    else:
...
def learner_setup(
    ...
    if config.network.memory_config.timestep_chunk_size:
        config.network.memory_config.chunk_size = config.network.memory_config.timestep_chunk_size * n_agents
    else:
    ...
    init_policy_hstate = ScannedRNN.initialize_carry((config.arch.num_envs, n_agents), config.network.hidden_state_dim)
    ...
    hstates = HiddenStates(init_hstates, init_policy_hstate)  # type: ignore
...
def run_experiment(_config: DictConfig) -> float:
    ...
    # PRNG keys.
    key, key_e, actor_net_key, net_key = jax.random.split(jax.random.PRNGKey(config.system.seed), num=4)
