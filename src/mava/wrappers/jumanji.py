...
class VectorConnectorWrapper(JumanjiMarlWrapper):
    ...
    def observe(self, state: State, timestep: TimeStep) -> TimeStep[Observation]:
        ...
        timestep_raw = timestep.replace(observation=observation, extras=timestep.extras["env_metrics"])
        timestep = self.modify_timestep(timestep_raw)
...
class CleanerWrapper(JumanjiMarlWrapper):
    ...
    def modify_timestep(self, timestep: TimeStep) -> TimeStep[Observation]:
        ...
        obs_data = {
            "agents_view": create_agents_view(timestep.observation.grid, timestep.observation.agents_locations),
            "action_mask": timestep.observation.action_mask,
            "step_count": jnp.repeat(timestep.observation.step_count, self.num_agents),
        }
        ...
        return timestep.replace(observation=Observation(**obs_data), reward=reward, discount=discount, extras=metrics)
