...
class RecordEpisodeMetrics(Wrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[RecordEpisodeMetricsState, TimeStep]:
        ...
        # Preserve existing extras (e.g., env_metrics from SmaxWrapper) and add episode_metrics
        existing_extras = timestep.extras or {}
        ...
        # Create new extras: preserve env_metrics and other keys, add/update episode_metrics
        new_extras = {**existing_extras, "episode_metrics": episode_metrics_dict}
...
class SFLRecordEpisodeMetrics(RecordEpisodeMetrics):
    def step(
        self,
        state: RecordEpisodeMetricsState,
        action: chex.Array,
        ...
        if reset_state is not None:
            env_state, timestep = self._env.step(state.env_state, action, reset_state.env_state)
        else:
            env_state, timestep = self._env.step(state.env_state, action)
