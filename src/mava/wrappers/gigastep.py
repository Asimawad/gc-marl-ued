...
class GigastepWrapper(Wrapper):
    ...
    def __init__(
        ...
        super().__init__(env)
        assert env.discrete_actions, "Only discrete action spaces are currently supported for Gigastep environments"
        assert env._obs_type == "vector", "Only Vector observations are currently supported for Gigastep environments"
    ...
    def observation_spec(self) -> specs.Spec:
        ...
        action_mask = specs.BoundedArray((self.num_agents, self._env.n_actions), float, 0.0, 1.0, "action_mask")
        step_count = specs.BoundedArray((self.num_agents,), jnp.int32, 0, self._env.max_episode_length, "step_count")
        if self.has_global_state:
