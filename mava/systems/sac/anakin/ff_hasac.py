...
def init(
    ...
    actor_action_head = hydra.utils.instantiate(action_head, action_dim=env.action_dim, independent_std=False)
    ...
    learner_state = LearnerState(first_obs, env_state, buffer_state, params, opt_states, t, first_keys)
...
def make_update_fns(
    ...
    # losses:
    def q_loss_fn(q_params: QVals, obs: Array, action: Array, target: Array) -> Tuple[Array, Metrics]:
    ...
    def update_q(
        ...
        new_target_q_params = optax.incremental_update(new_online_q_params, params.q.targets, cfg.system.tau)
        ...
        for _ in range(cfg.system.policy_update_delay):
            ...
            for agent_id in agent_ids:
                ...
                if cfg.system.autotune:
                    ...
                    new_alpha_opt_states = tree_at_set(opt_states.alpha, agent_id, new_alpha_opt_state)
    ...
    def act(
        ...
        actions, _ = get_actions(actor_params, actor_net, act_keys, env.num_agents, env.action_dim, obs)
    ...
    def explore(carry: LearnerState, _: Any) -> Tuple[LearnerState, Metrics]:
        ...
        t += cfg.arch.num_envs
        learner_state = carry._replace(obs=next_obs, env_state=env_state, buffer_state=buffer_state, t=t, key=key)
...
def run_experiment(cfg: DictConfig) -> float:
    ...
    def eval_act_fn(
        ...
        action, _ = get_actions(params, actor, keys, eval_env.num_agents, eval_env.action_dim, timestep.observation)
        return action, {}
