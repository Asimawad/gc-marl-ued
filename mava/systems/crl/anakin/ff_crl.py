...
# Import from local copies in CRL system
import sys
import os
...
@dataclass
class Args:
    """CRL training arguments for SMAX."""
    exp_name: str = "crl_smax"
    seed: int = 0
    checkpoint: bool = False
    
    # Neptune logging
    use_neptune: bool = True
    neptune_project: str = "InstaDeep/ssued"
    neptune_tags: str = "crl,smax,original"  # Comma-separated tags

    # Environment arguments (SMAX only)
    env_id: str = "smax"
...
def main(args: Args):
    """Main training function - can be called from Hydra wrapper or standalone."""
    ...
    if args.checkpoint:
        ...
        save_path = Path("./checkpoints") / Path(run_name)
    ...
    if args.use_neptune:
        try:
            ...
            neptune_run = neptune.init_run(
                ...
                name=run_name,
                tags=args.neptune_tags.split(","),
            )
            # Log hyperparameters
            neptune_run["parameters"] = {
                "seed": args.seed,
                "env_id": args.env_id,
                "smax_map_name": args.smax_map_name,
            ...
            print(f"[Neptune] Initialized run: {neptune_run.get_url()}")
        except Exception as e:
            print(f"[Neptune] Failed to initialize: {e}")
            neptune_run = None
    ...
    if args.env_id == "smax":
        ...
        raise ValueError(f"Only 'smax' environment is supported, got: {args.env_id}")
    
    # Get available actions for SMAX
    ...
    # Try to access the observation from the env_state
    try:
        obs = env_state.obs if hasattr(env_state, 'obs') else env_state
 
    except Exception as e:
        print(f"Could not access observation: {e}")
    print("="*80 + "\n")
    
    # Network setup
    # Initialize from scratch (no checkpoint loading)
    act_params = None
    crtc_params = None
    alph_params = None
...
#                jax.debug.print("actions on sampled obs: {}", means[:6,:])
    ...
    for ne in range(args.num_epochs):
        ...
        if neptune_run is not None:
            try:
                # Log all metrics to Neptune
                for metric_name, metric_value in metrics.items():
                    # Convert JAX arrays to Python scalars
                    if hasattr(metric_value, 'item'):
                        metric_value = metric_value.item()
                    neptune_run[metric_name].append(metric_value, step=training_state.env_steps.item())
            except Exception as e:
                print(f"[Neptune] Failed to log metrics: {e}")
    ...
    if neptune_run is not None:
        try:
            ...
            print(f"[Neptune] Failed to stop run: {e}")
        


if __name__ == "__main__":
    # Allow standalone execution with tyro
    args = tyro.cli(Args)
    main(args)
