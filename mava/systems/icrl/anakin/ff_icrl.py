...
from mava.types import MarlEnv, ExperimentOutput, Metrics
...
def get_learner_fn(
    ...
    def _env_step(
        ...
        def single_step(carry, _):
            ...
            flat_avail = avail_actions.reshape(num_envs_agents, -1) if avail_actions is not None else jnp.ones_like(flat_action)
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        # after: learner_state, episode_metrics = _env_step(learner_state, None)
        ...
        def _update_minibatch(carry, batch_transitions):
            ...
            def _actor_loss_fn(actor_params, critic_params, obs, alpha, avail_actions, future_state, key):
                ...
                if config.system.discrete_actions:
                    ...
                    # Compute effective discrete entropy
                    probs = jax.nn.softmax(action_logits)
                    log_probs_discrete = jax.nn.log_softmax(action_logits)
                    log_probs_discrete = jnp.clip(log_probs_discrete, a_min=-20.0)
                    effective_discrete_entropy = -(probs * log_probs_discrete).sum(axis=-1).mean()
                    num_available = avail_mask.sum(axis=-1).mean()
                    max_discrete_entropy = jnp.log(num_available)
                else:
                    stds = jnp.exp(log_stds)
                    ...
                    # Compute effective discrete entropy by analyzing the pre-tanh distribution
                    # After Gumbel noise, what's the probability of each action being selected?
                    masked_means = means.copy()  # Already masked above
                    masked_means = jnp.where(avail_mask == 0, -1e10, masked_means)
                    
                    # The discrete action is argmax(tanh(means + gumbel_noise))
                    # Approximate the distribution by looking at the softmax of means/temperature
                    # Temperature can be approximated by std
                    temperature = stds.mean()  # Average temperature
                    action_logits_approx = masked_means / (temperature + 1e-8)
                    probs = jax.nn.softmax(action_logits_approx, axis=-1)
                    log_probs_discrete = jax.nn.log_softmax(action_logits_approx, axis=-1)
                    log_probs_discrete = jnp.clip(log_probs_discrete, a_min=-20.0)
                    effective_discrete_entropy = -(probs * log_probs_discrete).sum(axis=-1).mean()
                    num_available = avail_mask.sum(axis=-1).mean()
                    max_discrete_entropy = jnp.log(num_available)
                ...
                loss_info = {
                    ...
                    "entropy": entropy,
                    "effective_discrete_entropy": effective_discrete_entropy,
                    "max_discrete_entropy": max_discrete_entropy,
                    "entropy_ratio": effective_discrete_entropy / (max_discrete_entropy + 1e-8),
                    "num_available_actions": num_available,
                }
...
def get_prefill_fn(env, buffer, actor_apply, config):
    ...
    def _rollout_once(learner_state):
        ...
        def single_step(carry, _):
            ...
            # Flatten obs for actor (matching CRL's structure)
            obs_flat = obs.reshape(-1, obs.shape[-1])  # [N_env*N_agent, obs_dim]
            means, log_stds = actor_apply(params.actor, obs_flat)  # [N_env*N_agent, action_dim]
            ...
            if hasattr(env, "action_mask"):
                ...
                avail_flat = avail.reshape(-1, avail.shape[-1])  # [N_env*N_agent, action_dim]
                avail = jnp.asarray(avail_flat, dtype=means.dtype)

            if config.system.discrete_actions:
                if avail is not None:
                    action_logits = means - ((1.0 - avail) * 1e10)
                else:
                    action_logits = means
                categorical = distrax.Categorical(logits=action_logits)
                action_idx = categorical.sample(seed=policy_key)
                stored_action = jax.nn.one_hot(action_idx, action_logits.shape[-1])
                env_actions = action_idx.reshape(num_envs, n_agents)
            else:
                # Gumbel-Softmax
                stds = jnp.exp(log_stds)
                
                if avail is not None:
                    means = means - ((1.0 - avail) * 1e10)
                
                means_reshaped = means.reshape(num_envs, n_agents, -1)
                means_for_noise = means_reshaped.transpose(1, 0, 2)
                
                noise_keys = jax.random.split(policy_key, n_agents)
                noise = jax.vmap(
                    lambda rng, m: jax.random.gumbel(
                        rng, shape=(num_envs,) + m.shape[1:], dtype=m.dtype
                    ),
                    in_axes=(0, 0),
                )(noise_keys, means_for_noise)
                noise_reshaped = noise.transpose(1, 0, 2).reshape(-1, noise.shape[-1])
                
                actions_flat = nn.tanh(means + stds * noise_reshaped)
                stored_action = actions_flat
                
                env_actions_flat = jnp.argmax(actions_flat, axis=-1)
                env_actions = env_actions_flat.reshape(num_envs, n_agents)
            ...
            if avail is not None:
                ...
                flat_avail = jnp.ones_like(flat_action)
...
def learner_setup(
    ...
    if hasattr(config.network, 'sa_encoder_network') and hasattr(config.network.sa_encoder_network, 'pre_torso'):
        ...
        # Fallback: default ICRL architecture (4x1024 layers with swish)
    ...
    n_agents = env.num_agents
    # breakpoint()
    # Observation dimensions from config (set dynamically in make_env.py)
    # For SMAX: base_obs_dim=127, goal_dim=2 → obs_size=129
    # For Connector: base_obs_dim=125, goal_dim=2 → obs_size=127
