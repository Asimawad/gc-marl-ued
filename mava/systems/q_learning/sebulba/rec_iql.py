...
def rollout(
    ...
    def select_eps_greedy_action(
        ...
        eps = jnp.maximum(config.system.eps_min, 1 - (t / config.system.eps_decay) * (1 - config.system.eps_min))
    ...
    hstate = ScannedRNN.initialize_carry((config.arch.num_envs, num_agents), config.network.hidden_state_dim)
...
def get_learner_step_fn(
    ...
    def _update_step(
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            if config.system.hard_update:
                ...
                next_target_params = optax.incremental_update(next_online_params, params.target, config.system.tau)
    ...
    def learner_fn(learner_state: LearnerState, traj_batch: Transition) -> Tuple[LearnerState, Metrics]:
        """Learner function.
...
def run_experiment(_config: DictConfig) -> float:
    ...
    # Setup learner.
    learn, q_net, learner_state, learner_sharding, init_transition = learner_setup(key, config, learner_devices)
    ...
    num_updates_per_insert = config.system.epochs * config.system.sample_batch_size * config.system.rollout_length
    ...
    config.system.sample_per_insert = (num_setps_per_insert * config.system.replay_ratio) / num_updates_per_insert
    ...
    for eval_step in range(config.arch.num_evaluation):
        ...
        train_metrics["learner_steps_per_second"] = (config.system.num_updates_per_eval) / time_metrics[
