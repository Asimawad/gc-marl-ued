...
class JaxMarlWrapper(Wrapper, ABC):
    ...
    def reset(
        ...
        state = JaxMarlState(
            ...
            won_battle=False,
            episode_seed=jnp.array(0, dtype=int)
        )
        metrics["seed"] = jnp.array(0.0, dtype=jnp.float32)
        metrics["truncation"] = jnp.array(0.0, dtype=jnp.float32)
    ...
    def step(
        ...
        reward_array = batchify(reward, self.agents)
        ts = TimeStep(
            step_type=step_type,
            reward=reward_array,
        ...
        # Increment seed on episode termination (for next episode)
        new_episode_seed = jax.lax.select(
            done["__all__"],  # If episode is ending
            state.episode_seed + 1,  # Increment seed for next episode
            state.episode_seed  # Keep same seed
        )
        
        # Add seed to extras for buffer relabeling
        seed_scalar = jnp.array(state.episode_seed, dtype=jnp.float32)
        metrics["seed"] = seed_scalar
        metrics["truncation"] = jnp.array(0.0, dtype=jnp.float32)
        
        state = JaxMarlState(
            env_state, 
            key, 
            state.step + jnp.array(1, dtype=int),
            step_won=state.step_won,
            won_battle=state.won_battle,
...
class SmaxWrapper(JaxMarlWrapper):
    ...
    def __init__(
        ...
        zero_rewards_for_icrl: bool = False,
    ):
        super().__init__(env, has_global_state, env.max_steps)
        self._env: SMAX
        self.zero_rewards_for_icrl = zero_rewards_for_icrl

    def reset(
        self, key: PRNGKey
    ) -> Tuple[JaxMarlState, TimeStep[Union[Observation, ObservationGlobalState]]]:
        state, ts = super().reset(key)
        won_battle_init = jnp.full((self.num_agents,), False, dtype=bool)
        won_episode_init = jnp.full((self.num_agents,), False, dtype=bool)
        
        existing_extras = ts.extras or {}
        existing_env_metrics = existing_extras.get("env_metrics", {})
        existing_env_metrics["won_episode"] = won_episode_init
        existing_env_metrics["won_battle"] = won_battle_init
        
        extras = {**existing_extras, "env_metrics": existing_env_metrics}
        extras["seed"] = jnp.array(state.episode_seed, dtype=jnp.float32)
        extras["truncation"] = jnp.array(0.0, dtype=jnp.float32)
    ...
    def step(
        ...
        # Check reward to detect wins (sparse rewards: reward > 0 only when battle is won)
        reward_agent0 = reward[self.agents[0]]
        won_this_step = (reward_agent0 > 0).astype(bool)
        new_won_battle_step = won_this_step
        state_won_battle = state.won_battle | new_won_battle_step
        
        # Track the step when battle was first won
        step_won_current = jnp.array(state.step_won, dtype=jnp.float32)
        ...
        current_step_float = jnp.array(state.step, dtype=jnp.float32)
        new_step_won = jax.lax.select(
            state_won_battle & (step_won_after_reset == zero_float),
            current_step_float,
            step_won_after_reset
        )
        
        state = state.replace(step_won=new_step_won, won_battle=state_won_battle)
        
        metrics: Dict[str, Any] = {"env_metrics": {}}
        obs_obj = self._create_observation(obs, env_state)
        obs_obj = obs_obj._replace(step_count=jnp.repeat(state.step, self.num_agents))
        step_type = jax.lax.select(done["__all__"], StepType.LAST, StepType.MID)
        reward_array = batchify(reward, self.agents)
        
        # Win repeat logic: freeze state for 5 steps after win
        steps_since_won = state.step.astype(jnp.float32) - new_step_won
        win_repeat = 5
        
        original_done = step_type == StepType.LAST
        new_done = jax.lax.select(state_won_battle, False, original_done)
        new_done = jax.lax.select(
            state_won_battle & (steps_since_won > win_repeat),
            True,
            new_done
        )
        
        time_limit_reached = state.step >= self.time_limit
        new_done = jax.lax.select(time_limit_reached, True, new_done)
        
        new_step_type = jax.lax.select(new_done, StepType.LAST, StepType.MID)
        won_episode = state_won_battle & (new_step_type == StepType.LAST)
        
        won_battle_per_agent = jnp.full((self.num_agents,), new_won_battle_step, dtype=bool)
        won_episode_per_agent = jnp.full((self.num_agents,), won_episode, dtype=bool)
        
        seed_scalar = jnp.array(state.episode_seed, dtype=jnp.float32)
        truncation_scalar = jnp.array(0.0, dtype=jnp.float32)
        
        metrics = {
            "env_metrics": {
                "won_episode": won_episode_per_agent,
                "won_battle": won_battle_per_agent
            },
            "seed": seed_scalar,
            "truncation": truncation_scalar
        }
        
        # Zero rewards for ICRL (sparse reward setting)
        final_reward = jax.lax.cond(
            self.zero_rewards_for_icrl,
        ...
        # Freeze environment state when won (prevents evolution during win_repeat period)
        final_env_state = jax.tree_util.tree_map(
            lambda old, new: jax.lax.select(won_this_step, old, new),
            state.state,
            env_state
        )
        
        new_state = JaxMarlState(
            final_env_state,
            key, 
            state.step + jnp.array(1, dtype=int),
            step_won=new_step_won,
            won_battle=state_won_battle,
    ...
    def action_dim(self) -> chex.Array:
        ...
        if _is_discrete(single_agent_action_space):
            return single_agent_action_space.n
        else:  # Box (continuous)
            return single_agent_action_space.shape[0]
...
class MPEWrapper(JaxMarlWrapper):
    ...
    def base_obs_dim(self) -> int:
        """Get the base observation size from JaxMarl (before any ICRL modifications).
        
        This is the observation size that JaxMarl gives us, works for ANY MPE scenario.
        """
        return self._env.observation_space(self.agents[0]).shape[0]
    
    @cached_property
    def icrl_info(self) -> Dict[str, int]:
        """Provide ICRL-relevant information about observations.
        
        Returns dictionary with:
        - base_obs_dim: Size of JaxMarl observation
        - current_goal_size: Expected size of current goal (1 for MPE)
        - goal_size: Size of goal (1 for ICRL)
        """
        return {
            "base_obs_dim": self.base_obs_dim,
            "current_goal_size": 1,  # MPE current goal is always 1D (min_distance)
            "goal_size": 1,  # ICRL always adds 1 goal
        }
