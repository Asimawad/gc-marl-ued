...
from mava.wrappers import (
    AgentIDWrapper,
    AutoResetWrapper,
    # CleanerWrapper,  # TODO: Missing - commented out
    ConnectorWrapper,
    GigastepWrapper,
    GymAgentIDWrapper,
    GymRecordEpisodeMetrics,
    GymToJumanji,
    # LbfWrapper,  # TODO: Missing - commented out
    MabraxWrapper,
    MatraxWrapper,
    MPEWrapper,
    RecordEpisodeMetrics,
    # RwareWrapper,  # TODO: Missing - commented out
    SmacWrapper,
    SmaxWrapper,
    UoeWrapper,
...
_jumanji_registry: registry_type = {
    # "RobotWarehouse": {"generator": RwareRandomGenerator, "wrapper": RwareWrapper},  # TODO: Missing wrapper
    # "LevelBasedForaging": {"generator": LbfRandomGenerator, "wrapper": LbfWrapper},  # TODO: Missing wrapper
    ...
    # "Cleaner": {"generator": CleanerRandomGenerator, "wrapper": CleanerWrapper},  # TODO: Missing wrapper
}
...
def add_extra_wrappers(
    ...
    if config.system.get("use_icrl", False):
        # Extract base observation dimension from environment
        obs_spec = train_env.observation_spec
        base_obs_dim = obs_spec.agents_view.shape[-1]
        
        # Goal dimension is always 2: [current_progress, ultimate_goal]
        # - current_progress: dynamic value (e.g., sum_enemy_health, manhattan_distance)
        # - ultimate_goal: constant target value (always 0.0 for both SMAX and Connector)
        goal_dim = 2
        
        # Compute derived dimensions for ICRL
        # obs_dim: dimension used in loss functions (base + current_progress, excludes ultimate_goal)
        # goal_start_idx: where goal starts in the observation vector
        # goal_end_idx: where to slice for hindsight relabeling (only first dim of goal)
        ...
        config.system.icrl.goal_end_idx = base_obs_dim + 1  # Only extract first dimension for relabeling
        
        # Get goal_type from config (default to "manhattan_distance" for backward compatibility)
        goal_type = config.system.icrl.get("goal_type", "manhattan_distance")
        
        train_env = ICRLGoalWrapper(train_env, config.env.env_name, config.system.icrl.obs_dim, config.system.icrl.goal_dim, config.system.icrl.goal_start_idx, config.system.icrl.goal_end_idx, goal_type=goal_type)
        eval_env = ICRLGoalWrapper(eval_env, config.env.env_name, config.system.icrl.obs_dim, config.system.icrl.goal_dim, config.system.icrl.goal_start_idx, config.system.icrl.goal_end_idx, goal_type=goal_type)
...
def make_jaxmarl_env(config: DictConfig, add_global_state: bool = False) -> Tuple[MarlEnv, MarlEnv]:
    ...
    if "smax" in config.env.env_name.lower():
        # Check if we should use the original ICRL environment via Brax adapter

            kwargs["scenario"] = map_name_to_scenario(config.env.scenario.task_name)
            # Check if ICRL is enabled to use sparse rewards (matching CRL behavior)
            # CRL's OLD_smax_env.py returns only won_battle_bonus (sparse rewards)
            # This allows checking reward > 0 to detect wins reliably
            use_sparse_rewards = (
                hasattr(config.system, 'use_icrl') 
                and config.system.use_icrl 
            )
            if use_sparse_rewards:
                kwargs["use_sparse_rewards"] = True
            train_base_env = jaxmarl.make(config.env.scenario.name, **kwargs)
            eval_base_env = jaxmarl.make(config.env.scenario.name, **kwargs)
            # Zero rewards in wrapper after checking for wins (CRL behavior)
            zero_rewards = use_sparse_rewards
            train_env: MarlEnv = _jaxmarl_registry[config.env.env_name]["wrapper"](
                train_base_env,
                add_global_state,
                zero_rewards_for_icrl=zero_rewards,
            )
