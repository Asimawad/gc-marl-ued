...
def validate_num_dims_in_graph_tuple(graph: GraphsTuple, num_batch_dims: int) -> None:
    ...
    assert graph.ego_node_index.ndim == num_ego_node_index_dims, (
        "The number of batch dimensions in the ego node index must match the number of" " batch dimensions"
    )
    assert graph.n_node.ndim == num_n_node_dims, (
        "The number of batch dimensions in the number of nodes must match the number of" " batch dimensions"
    )
    assert graph.n_edge.ndim == num_n_edge_dims, (
        "The number of batch dimensions in the number of edges must match the number of" " batch dimensions"
    )
...
def batch(graphs: List[GraphsTuple]) -> GraphsTuple:
    ...
    return GraphsTuple(
        ...
        ego_node_index=jnp.concatenate([g.ego_node_index + o for g, o in zip(graphs, offsets, strict=False)]),
    )
