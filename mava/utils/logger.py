...
def winrate_custom_metric(metrics: Metrics) -> Metrics:
    ...
    is_terminal_steps = metrics.get("is_terminal_step", None)
    if is_terminal_steps is not None:
        # Use is_terminal_step if available (preferred method)
        # Handle both numpy arrays and JAX arrays
        # Check if it's an array-like object (JAX array, numpy array, or list)
        if hasattr(is_terminal_steps, '__array__') or isinstance(is_terminal_steps, (np.ndarray, list, tuple)):
            # Convert to numpy array for safe operations
            # This handles both JAX arrays and numpy arrays
            is_terminal_steps_np = np.asarray(is_terminal_steps)
            n_episodes: int = int(np.sum(is_terminal_steps_np))
        else:
            # Single scalar value (bool or int)
            # Use direct conversion, but avoid boolean context for arrays
            try:
                n_episodes: int = int(is_terminal_steps) if bool(is_terminal_steps) else 0
            except (ValueError, TypeError):
                # Fallback: if conversion fails, assume it's an array that needs summing
                is_terminal_steps_np = np.asarray(is_terminal_steps)
                n_episodes: int = int(np.sum(is_terminal_steps_np))
    else:
        # Fallback: count episodes from won_episode length (if is_terminal_step was removed)
        won_episode = metrics["won_episode"]
        if isinstance(won_episode, np.ndarray):
            n_episodes = won_episode.size if won_episode.ndim > 0 else 1
        elif isinstance(won_episode, (list, tuple)):
            n_episodes = len(won_episode)
        else:
            n_episodes = 1
    ...
    if hasattr(won_episode, '__array__') or isinstance(won_episode, (np.ndarray, list, tuple)):
        # Convert to numpy array for safe operations (handles both JAX and numpy arrays)
        won_episode_np = np.asarray(won_episode)
        n_won_episodes: int = int(np.sum(won_episode_np))
    else:
        # Single scalar value (bool or int)
        try:
            n_won_episodes: int = int(won_episode) if bool(won_episode) else 0
        except (ValueError, TypeError):
            # Fallback: convert to array and sum
            won_episode_np = np.asarray(won_episode)
            n_won_episodes: int = int(np.sum(won_episode_np))
    win_rate: float = (n_won_episodes / n_episodes) * 100

    # Update metrics: add win_rate and remove won_episode
    metrics["win_rate"] = win_rate
    metrics.pop("won_episode", None)
...
class ConsoleLogger(BaseLogger):
    ...
    def log_dict(self, data: Metrics, step: int, eval_step: int, event: LogEvent) -> None:
        ...
        # Filter out redundant/verbose metrics for cleaner console output
        if event == LogEvent.EVAL:
            # Remove redundant metrics (keep only the most informative ones)
            redundant_keys = [
                "eval/episode success any mean",
                "eval/episode success any std", 
                "eval/episode success any min",
                "eval/episode success any max",
                "won battle mean",
                "won battle std",
                "won battle min",
                "won battle max",
            ]
            data = {k: v for k, v in data.items() if k not in redundant_keys}
            
            # Organize metrics in a logical order: performance → episode stats → env-specific
            priority_order = [
                "steps per second",
                "win rate",
                "episode return mean",
                "episode length mean",
                "num connections mean",  # Connector-specific
                "ratio connections mean",  # Connector-specific
            ]
            
            # Reorder: priority metrics first, then the rest
            ordered_data = {}
            for key_pattern in priority_order:
