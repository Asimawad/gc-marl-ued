...
class TrajectoryUniformSamplingQueue():
    ...
    def insert_internal(
        ...
        current_size =  buffer_state.insert_position - buffer_state.sample_position
    ...
    def flatten_crl_fn(buffer_config, transition, sample_key):
        ...
        # Debug: Check seed equality mask
        ...
        future_state_sampled = jnp.take(transition.observation, goal_index[:-1], axis=0) #the last goal_index cannot be considered as there is no future.  
        ...
        return transition._replace(
            ...
            avail_actions=jnp.squeeze(transition.avail_actions[:-1]),  # Preserve avail_actions mask
            extras=extras,
        )
