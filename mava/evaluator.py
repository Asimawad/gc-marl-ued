...
def get_eval_fn(
    ...
    def eval_fn(params: FrozenDict, key: PRNGKey, init_act_state: ActorState) -> Metrics:
        ...
        def _episode(key: PRNGKey, _: Any) -> Tuple[PRNGKey, Metrics]:
            ...
            episode_metrics = timesteps.extras["episode_metrics"]
            env_metrics = timesteps.extras["env_metrics"]
            
            won_episode = env_metrics["won_episode"]
            
            if won_episode.ndim == 3:
                won_episode_per_episode = jnp.any(won_episode, axis=-1)
            elif won_episode.ndim == 2:
                if won_episode.shape[-1] <= 10:
                    won_episode_per_episode = jnp.any(won_episode, axis=-1, keepdims=True)
                else:
                    won_episode_per_episode = won_episode if won_episode.ndim == 2 else won_episode[:, jnp.newaxis]
            else:
                won_episode_per_episode = won_episode
                if won_episode_per_episode.ndim == 1:
                    won_episode_per_episode = won_episode_per_episode[:, jnp.newaxis]
            
            if won_episode_per_episode.ndim == 2:
                won_episode = jnp.any(won_episode_per_episode, axis=0)
            else:
                won_episode = jnp.array([jnp.any(won_episode_per_episode)])
            
            T = timesteps.last().shape[0]
            won_episode_broadcast = jnp.broadcast_to(won_episode[jnp.newaxis, :], (T, won_episode.shape[0]))
            
            metrics = episode_metrics | env_metrics
            metrics["won_episode"] = won_episode_broadcast
        ...
        metrics = tree.map(lambda x: x.reshape(-1), metrics)
        
        if "won_episode" in metrics:
            success = metrics["won_episode"].astype(jnp.float32)
            metrics["eval/episode_success_any"] = jnp.mean(success)
...
def make_ff_eval_act_fn(actor_apply_fn: ActorApply, config: DictConfig) -> EvalActFn:
    ...
    def eval_act_fn(
        ...
        if getattr(config.system, "use_icrl", False):
            obs_view = observation.agents_view
            action_mask = getattr(observation, "action_mask", None)

            # Check if using discrete ICRL (ff_icrl_discrete_sac.py)
            # Discrete actor returns distribution directly, continuous returns (means, log_stds)
            use_discrete_icrl = getattr(config.system, "use_discrete_icrl", False)
            
            if use_discrete_icrl:
                # Discrete ICRL: actor returns distribution with action masking built-in
                # If no action mask provided, create one that allows all actions
                if action_mask is None:
                    # Infer action_dim from observation shape or use a large default
                    # This will be caught if actor is called with wrong shape
                    action_mask = jnp.ones(obs_view.shape[:-1] + (obs_view.shape[-1],), dtype=jnp.bool_)
                policy = actor_apply_fn(params, obs_view, action_mask)
                action = policy.mode() if config.arch.evaluation_greedy else policy.sample(seed=key)
            else:
                # Continuous ICRL: actor returns (means, log_stds)
                means, log_stds = actor_apply_fn(params, obs_view)

                if config.system.discrete_actions:
                    if action_mask is not None:
                        mask = action_mask.astype(means.dtype)
                        means = means - ((1.0 - mask) * 1e10)
                    action = jnp.argmax(means, axis=-1)
                else:
                    if action_mask is not None:
                        mask = action_mask.astype(means.dtype)
                        means = means - ((1.0 - mask) * 1e10)
                    action_continuous = nn.tanh(means)
                    action = jnp.argmax(action_continuous, axis=-1)

            return action, {}

        pi = actor_apply_fn(params, observation)
...
def get_sebulba_eval_fn(
    ...
    def eval_fn(params: FrozenDict, key: PRNGKey, init_act_state: ActorState) -> Metrics:
        ...
        def _episode(key: PRNGKey) -> Tuple[PRNGKey, Metrics]:
            ...
            if "env_metrics" in timesteps.extras:
                env_metrics = timesteps.extras["env_metrics"]
                if config.env.log_win_rate:
                    if "won_episode" in env_metrics:
                        won_episode_per_agent = env_metrics["won_episode"]
                        
                        if won_episode_per_agent.ndim == 3:
                            won_episode_per_episode = np.any(won_episode_per_agent, axis=-1)
                            won_episode_at_done = won_episode_per_episode[done_idx, np.arange(n_parallel_envs)]
                        elif won_episode_per_agent.ndim == 2:
                            won_episode_per_episode = np.any(won_episode_per_agent, axis=-1)
                            if n_parallel_envs == 1:
                                won_episode_at_done = np.array([won_episode_per_episode[done_idx[0]]])
                            else:
                                won_episode_at_done = np.take(won_episode_per_episode, done_idx)
                        else:
                            won_episode_at_done = won_episode_per_agent
                        
                        metrics["won_episode"] = won_episode_at_done
...
# metrics = {
#     "seed": jnp.asarray(episode_seed, dtype=jnp.float32),  # NEW!
#     "truncation": truncation,                               # NEW!
#     "env_metrics": {
#         "won_episode": won_episode,
#         **timestep.extras,
#     }
# }