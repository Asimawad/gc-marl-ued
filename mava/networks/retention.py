...
class SimpleRetention(nn.Module):
    ...
    def __call__(self, key: Array, query: Array, value: Array, hstate: Array, dones: Array) -> Tuple[Array, Array]:
        ...
        if self.memory_config.type == "ff_sable":
            ...
            next_hstate = (k_proj @ (v_proj * decay_matrix[:, -1].reshape((B, C, 1)))) + hstate * chunk_decay * delta
    ...
    def get_decay_matrix(self, dones: Array) -> Array:
        ...
        # B, T, T ->  B, T * N, T * N
        decay_matrix = jnp.repeat(jnp.repeat(decay_matrix, self.n_agents, axis=1), self.n_agents, axis=2)
...
class MultiScaleRetention(nn.Module):
    ...
    def __call__(
        ...
        for head in range(self.n_head):
            ...
            ret_output = ret_output.at[:, :, self.head_size * head : self.head_size * (head + 1)].set(y)
            hstate = hstate.at[:, head, :, :].set(new_hs)

        ret_output = self.group_norm(ret_output.reshape(-1, self.head_size)).reshape(ret_output.shape)
    ...
    def recurrent(
        ...
        for head in range(self.n_head):
            y, new_hs = self.retention_heads[head].recurrent(key_n, query_n, value_n, hstate[:, head])
            ret_output = ret_output.at[:, :, self.head_size * head : self.head_size * (head + 1)].set(y)
            hstate = hstate.at[:, head, :, :].set(new_hs)
