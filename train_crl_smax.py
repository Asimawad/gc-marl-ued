...
@dataclass
class Args:
    exp_name: str = "crl_with_temperature"
    seed: int = 1
    ...
    initial_temperature: float = 1.0  # Start with high exploration
    min_temperature: float = 0.01    # Prevent temperature from collapsing to 0
    target_entropy_ratio: float = 0.5  # target = ratio * max_entropy (for discrete actions)
...
class TrainingState:
    """Contains training state for the learner - with learned temperature like SAC's alpha."""
    env_steps: jnp.ndarray
    gradient_steps: jnp.ndarray
    critic_state: TrainState
    target_critic_params: Any
    temperature_state: TrainState  # Learned temperature (like SAC's alpha_state)
...
if __name__ == "__main__":
    ...
    # Temperature learning (like SAC's alpha) - we learn log_temperature for stability
    # For discrete actions: max_entropy = log(num_actions), target is a fraction of that
    max_entropy = jnp.log(action_size)  # ~2.4 for 11 actions
    target_entropy = args.target_entropy_ratio * max_entropy  # e.g., 0.5 * 2.4 = 1.2
    print(f"Target entropy: {target_entropy:.4f} (max: {max_entropy:.4f}, ratio: {args.target_entropy_ratio})")
    ...
    # Training state - with learned temperature like SAC
    ...
    def deterministic_actor_step(training_state, env, env_state, extra_fields):
        ...
        logits = compute_q_values(training_state.target_critic_params, state, goal)
    ...
    def actor_step(target_critic_params, env, env_state, key, log_temperature, extra_fields):
        """Training data collection step - sample actions from TARGET critic with learned temperature."""
        obs = jnp.reshape(env_state.obs, (-1,) + env_state.obs.shape[2:])

        state = obs[:, :args.obs_dim]
        goal = obs[:, args.obs_dim:]

        logits = compute_q_values(target_critic_params, state, goal)

        avail_actions = get_avail_act(env_state)
        avail_actions = jnp.reshape(avail_actions, (-1,) + avail_actions.shape[2:])
        masked_logits = logits - ((1 - avail_actions) * 1e10)

        # Use learned temperature (exponentiate log_temperature)
    ...
    @jax.jit
    def get_experience(target_critic_params, env_state, buffer_state, key, log_temperature):
        @jax.jit
    ...
    def update_critic(transitions, training_state, key):
        ...
        # Polyak update target towards the NEW online params
    ...
    def update_temperature(transitions, training_state):
        """
        Update temperature to maintain target entropy (like SAC's alpha update).
        This is the key insight: we learn the exploration-exploitation balance.
        
        For discrete actions with POSITIVE target entropy:
        - If entropy < target (too greedy): increase temperature
        - If entropy > target (too random): decrease temperature
        """
        def temperature_loss(temp_params, transitions):
            log_temperature = temp_params["log_temperature"]
            # Clamp log_temperature to ensure minimum temperature
            ...
            # Compute current policy (softmax of Q/temperature)
            policy = jax.nn.softmax(masked_q / temperature, axis=-1)
            
            # Compute policy entropy: H = -sum(p * log(p))
            # Only for available actions
            log_policy = jnp.log(policy + 1e-8)
            entropy = -jnp.sum(policy * log_policy * avail_actions, axis=-1)
            
            # Temperature loss: push entropy toward target
            # We want to MINIMIZE (target_entropy - entropy)^2, but use a simpler linear loss:
            # If entropy < target: we want to INCREASE temperature, so loss should have negative gradient w.r.t. log_temp
            # If entropy > target: we want to DECREASE temperature, so loss should have positive gradient w.r.t. log_temp
            # 
            # Loss = log_temperature * (entropy - target_entropy)
            # d(Loss)/d(log_temperature) = (entropy - target_entropy)
            # If entropy < target: gradient is negative -> log_temp increases -> temp increases (correct!)
            # If entropy > target: gradient is positive -> log_temp decreases -> temp decreases (correct!)
        ...
        # Also clamp the parameter after update to enforce minimum
    ...
    def sgd_step(carry, transitions):
        ...
        # Update critic (learns Q-values)
        training_state, critic_metrics = update_critic(transitions, training_state, critic_key)
        
        # Update temperature (learns exploration-exploitation balance)
    ...
    for ne in range(args.num_epochs):
        ...
        print(f" temperature : {metrics['training/temperature']:.4f} (target_entropy: {target_entropy:.2f})")
