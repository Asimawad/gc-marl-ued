"""
PQN (Parallelized Q-Network) for SMAX with LEARNABLE TEMPERATURE

Key improvements over basic PQN:
1. Learnable temperature (like SAC's alpha) for automatic exploration tuning
2. Entropy-based temperature adjustment
3. Layer normalization for training stability
4. No replay buffer - trains on fresh experience

This combines PQN's online learning with SAC's adaptive exploration.
"""
...
@dataclass
class Args:
    exp_name: str = "pqn_smax_learnable_temp"
    seed: int = 69
    ...
    num_epochs: int = 500
    num_envs: int = 512  # More parallel envs for diversity
    ...
    # Learnable temperature parameters (NEW!)
...
class SA_encoder(nn.Module):
    """State-Action encoder with LayerNorm for PQN stability."""
    rep_size: int
...
class TrainingState:
    """Training state for PQN with learnable temperature."""
    env_steps: jnp.ndarray
    gradient_steps: jnp.ndarray
    critic_state: TrainState
    target_critic_params: Any
    temperature_state: TrainState  # NEW!
...
if __name__ == "__main__":
    ...
    print(f"PQN Mode with LEARNABLE TEMPERATURE:")
    print(f"  - No replay buffer, training on fresh experience")
    ...
    def deterministic_actor_step(training_state, env, env_state, extra_fields, key):
        """Evaluation step - use current temperature for sampling."""
        ...
        # Use current learned temperature
        temperature = jnp.exp(training_state.temperature_state.params["log_temperature"])
        # actions = jax.random.categorical(key, logits / temperature, axis=-1)
    ...
    def actor_step(action_params, log_temperature, env, env_state, key, extra_fields):
        """Training step - sample actions with learned temperature."""
        obs = jnp.reshape(env_state.obs, (-1,) + env_state.obs.shape[2:])
    ...
    @jax.jit
    def collect_experience(action_params, log_temperature, env_state, key):
        """Collect experience from parallel envs."""
        @jax.jit
    ...
    def update_temperature(transitions, training_state):
        """Update temperature to maintain target entropy (SAC-style)."""
        def temperature_loss(temp_params, transitions, critic_params):
            log_temperature = temp_params["log_temperature"]
            log_temperature = jnp.clip(
                log_temperature,
            ...
            # Compute policy entropy under current Q-values
            ...
            # SAC-style loss: treat entropy as constant (stop_gradient)
        ...
        (temp_loss, entropy), temp_grad = jax.value_and_grad(temperature_loss, has_aux=True)(
            training_state.temperature_state.params,
            transitions,
    ...
    def update_critic_pqn(transitions, training_state, key):
        """Update critic on fresh experience."""
    ...
    def sgd_step(carry, transitions):
        ...
        # Update critic
        training_state, critic_metrics = update_critic_pqn(transitions, training_state, critic_key)
    ...
    def training_step(training_state, env_state, key):
        ...
        # Process transitions for CRL training
    ...
    training_walltime = 0
    print('Starting PQN training with LEARNABLE TEMPERATURE....')
    for ne in range(args.num_epochs):
        ...
        print(f" temperature : {metrics['training/temperature']:.4f} (target entropy: {target_entropy:.2f})")
