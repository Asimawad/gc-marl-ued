...
class Args:
    ...
    num_epochs: int = 500
    num_envs: int = 512  # More envs for PQN since no buffer
    ...
    unroll_length: int = 32  # Shorter unrolls, more frequent updates
...
if __name__ == "__main__":
    ...
    def process_trajectories_for_crl(transitions, key):
        """
        Process collected trajectories into (state, action, goal) tuples for CRL training.
        Similar to flatten_crl_fn but adapted for PQN.
        """
        # transitions has shape (unroll_length, num_envs_agents, ...)
        # We need to create (s, a, g) pairs where g is a future state
        
        unroll_len = transitions.observation.shape[0]
        num_envs = transitions.observation.shape[1]
        
        # Flatten time and env dimensions
        obs = transitions.observation.reshape(-1, transitions.observation.shape[-1])
        actions = transitions.action.reshape(-1)
        avail_actions = transitions.avail_actions.reshape(-1, transitions.avail_actions.shape[-1])
        
        # For goals: sample future observations from the same batch
        # This is a simple approach - sample random goals from the batch
        goal_indices = jax.random.randint(key, (obs.shape[0],), 0, obs.shape[0])
        goals = obs[goal_indices, args.obs_dim:]  # Extract goal part
        
        # Create processed transitions
        processed = Transition(
            observation=jnp.concatenate([obs[:, :args.obs_dim], goals], axis=-1),
            action=actions,
            avail_actions=avail_actions,
            reward=transitions.reward.reshape(-1),
            discount=transitions.discount.reshape(-1),
            extras=(),
        )
    ...
    def training_step(training_state, env_state, key):
        ...
        experience_key, train_key, goal_key = jax.random.split(key, 3)
        ...
        # Process into CRL format
        processed_transitions = process_trajectories_for_crl(transitions, goal_key)
        
        # Train immediately on fresh data
        training_state, metrics = update_critic_pqn(
            processed_transitions, training_state, train_key
        )
