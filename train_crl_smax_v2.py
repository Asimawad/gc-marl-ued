"""
CRL-DQN with Explicit Action Encoding (Option A)

Key difference from v1: The SA_encoder takes (state, action_onehot) as input,
exactly like SAC's critic. This gives the action full representational power
through all layers, instead of just being an output selector.

This should match SAC's performance more closely.
"""
...
@dataclass
class Args:
    exp_name: str = "crl_v2_explicit_action-target_entropy_ratio_0.1-initial_temperature_0.3"
    seed: int = 1
...
class SA_encoder(nn.Module):
    ...
    def __call__(self, s: jnp.ndarray, a: jnp.ndarray):
        ...
        Args:
            ...
            a: one-hot action, shape (batch, action_size)
        Returns:
...
class Transition(NamedTuple):
    ...
    action: jnp.ndarray  # Discrete action index
    reward: jnp.ndarray
...
if __name__ == "__main__":
    ...
    # Network setup - SA_encoder now takes (state, action_onehot)
    sa_encoder = SA_encoder(rep_size=args.rep_size)
    # Initialize with state + action_onehot input
    sa_encoder_params = sa_encoder.init(
        sa_key, 
        np.ones([1, args.obs_dim]),      # state
        np.ones([1, action_size])         # action_onehot
    )
    ...
    def compute_q_values(critic_params, obs, goal):
        """
        Compute Q-values for all actions by looping through each action.
        This is the key difference from v1 - we do num_actions forward passes.
        
        Returns: Q-values of shape (batch, action_size)
        """
        ...
        # Get goal representation once (shared across all actions)
        g_repr = g_encoder.apply(g_encoder_params, goal)  # (batch, rep_size)
        ...
        def compute_q_for_action(action_idx):
            ...
            # Get representation for (state, action) pair
            sa_repr = sa_encoder.apply(sa_encoder_params, obs, a_onehot)  # (batch, rep_size)
            
            # Q-value = negative L2 distance to goal
            q = -jnp.sqrt(jnp.sum((sa_repr - g_repr) ** 2, axis=-1))  # (batch,)
            return q
        
        # Vectorize over all actions
        q_values = jax.vmap(compute_q_for_action)(jnp.arange(action_size))  # (action_size, batch)
        q_values = q_values.T  # (batch, action_size)
    ...
    def actor_step(target_critic_params, env, env_state, key, log_temperature, extra_fields):
        """Training step - sample actions with temperature."""
        obs = jnp.reshape(env_state.obs, (-1,) + env_state.obs.shape[2:])
        ...
        actions = jax.random.categorical(key, masked_logits / 0.0375, axis=-1)
    ...
    def update_critic(transitions, training_state, key):
        def critic_loss(critic_params, transitions, key):
            ...
            action_onehot = jax.nn.one_hot(action, action_size)  # (batch, action_size)
            
            # Get representation for (state, action) pair - action flows through all layers!
            sa_repr = sa_encoder.apply(sa_encoder_params, obs, action_onehot)  # (batch, rep_size)
    ...
    training_walltime = 0
    print('Starting training (v2 - explicit action encoding)....')
