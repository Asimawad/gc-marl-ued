...
class MPETagFacmac6a(Env):
    def __init__(self):
        # Paper spec: 6 predators, 1 prey, 2 landmarks (not SimpleFacmacMPE6a which has 2 prey, 4 landmarks)
        self.env = simple_facmac.SimpleFacmacMPE(num_adversaries=6, num_good_agents=1, num_landmarks=2)
    ...
    def get_obs(self, state, obs):
        def get_agent_min(agent_obs):
            ...
            adv_pos = jp.concatenate((jp.zeros(2), adv_pos))
            ...
            dist_mat = jp.where(good_hidden, 99999999, dist_mat)
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        obs, env_state = self.env.reset(key1)
        ...
        state = State(env_state, self.get_obs(env_state.env_state, obs), reward, done, metrics)
