...
class TrajectoryUniformSamplingQueue():
    ...
    def flatten_crl_fn(buffer_config, transition, sample_key):
        ...
        gamma, goal_start_channel, goal_end_channel = buffer_config
        ...
        # goal = jnp.take(transition.observation, goal_index[:-1], axis=0)[:, :, :, goal_start_channel : goal_end_channel] # the last goal_index cannot be considered as there is no future.  
        goal = jnp.take(transition.extras["state_extras"]["goal"], goal_index[:-1], axis=0)
        
        state = transition.observation[:-1]
